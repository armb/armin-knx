...
unsafe {
	...
	println!("Packet '{:?}' sent: {}", packet, len);
    }
}
...
//     let con: *mut libc::c_void;
    let mut bus = Bus { value: 123., con: std::ptr::null_mut() };
...
//    let u1 = u.try_clone().expect("try_clone() failed");
    let _s = std::thread::spawn(move || bus_send_thread(rx, bus));
